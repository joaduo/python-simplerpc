  ...
  function ClassDefinition(context){
    ...
    function RPCCommandsQueue(){
      ...
      function syncCallback(data){
        ...
        if (!commands_results.length == commands_queue.length){ //Real error, answer should be of same length
          context.log.d('Receiving different amount of results ('+ commands_results.length +') for sent commands ('+ commands_queue.length +')');  
        }
        else if(commands_queue.length == 0){
          throw new Error('There were no commands. Then there sholdn\'t be any RPC.');
        }
        else{
          for ( var index = 0; index < commands_results.length; index++) {
            ...
            if(cmd_result['error'] == null){
              //call associated callback
              if(typeof callback !== 'undefined'){
                context.log.d('Calling command "'+ commands_queue[index][0] +'" at index '+ index+' command callback "' + RPCConnection.formatForLog(callback));
                callback(cmd_result['return_value']);
              }
              else{
                context.log.d('No callback for command "'+commands_queue[index][0] +'" at index: '+ index);
              }
            }
            else{ //There was an error, call the error_callback with data sent in the error (may be a message or something else)
              //call associated error_callback
              if(typeof error_callback !== 'undefined'){
                context.log.d('Calling command error_callback ' + RPCConnection.formatForLog(error_callback));
                error_callback(cmd_result['error']);
              }
              else{
                context.log.d('Error for RPC Command"'+ commands_queue[index][0] +'" number '+ index +' in the batch. No error_callback associated. Result Error: "'+cmd_result['error']+'"');
              }
            }
          }
        }
      ...
      this.sync = function(){
        ...
        RPCConnection.asyncGameServerRPC('/commands_queue/', '', post_args, syncCallback, errorCallback );
      };  
